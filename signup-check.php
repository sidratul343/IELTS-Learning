...
if (isset($_POST['uname']) && isset($_POST['password'])
	...
	if (empty($uname)) {
        ...
        header("Location: user_signup.php?error=Retyping Password is required&$user_data");
	    exit();
	}
		...
		if (mysqli_num_rows($result) > 0) {
			header("Location: user_signup.php?error=The username is taken try another&$user_data");
	        exit();
		}else {
           $sql2 = "INSERT INTO users(user_name, password, name, email) VALUES('$uname', '$pass', '$name', '$email')";
           $result2 = mysqli_query($conn, $sql2);
           if ($result2) {
           	 header("Location: user_login.php?success=Your account has been created successfully");
	         exit();
           }else {
	           	header("Location: user_signup.php?error=unknown error occurred&$user_data");
		        exit();
           }
		}
	}
